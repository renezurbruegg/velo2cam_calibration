...
/*
  ...
  the ArUco markers
*/
#include <velo2cam_utils.h> // Must be first import
...
ros::Publisher img_pub, qr_pub, centers_cloud_pub, cumulative_pub, clusters_pub;
...
void imageCallback(const sensor_msgs::ImageConstPtr &msg,
  ...
  std::vector<int> boardIds{0, 1, 3, 4}; // IDs order as explained above
...
// Estimate 3D position of the board using detected markers
    ...
    **/
    ...
    if (best_candidate_idx == -1) {
      ...
      ROS_WARN("[Mono] Unable to find a candidate set that matches target's "
               "geometry");
      return;
    }
    ...
    if (DEBUG || RVIZ_DEBUG) { // Draw centers
      for (int i = 0; i < centers_cloud->size(); i++) {
...
int main(int argc, char **argv) {
  ...
  img_pub = nh_.advertise<sensor_msgs::Image>("detected_pattern", 1);
  ...
  std::cout << "delta_width_circles" << delta_width_circles_ << "\n"
            << "delta_height_circles" << delta_height_circles_ << "\n"
            << "marker_size" << marker_size_ << "\n"
            << "delta_width_qr_center_" << delta_width_qr_center_ << "\n"
            << "delta_height_qr_center_" << delta_height_qr_center_ << "\n"
            << "min_detected_markers" << min_detected_markers_ << "\n";
